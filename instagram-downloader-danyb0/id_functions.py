...
def menu():
    ...
    #guida_button = tk.Button(menu, text=("Guide"), font=("Helvetica", 11), command=guida, bg=sfondo, fg=scritte)
    #guida_button.grid(row=0,column=0, padx=3, pady=3, sticky="nwe")
    #guida_button.configure(relief="flat")
  
    dire_button = tk.Button(menu, text=("Change Directory"), font=("Helvetica", 11), command=directory, bg=sfondo, fg=scritte, activebackground=sfondo, activeforeground=scritte)
    dire_button.grid(row=0,column=0, padx=3, pady=3, sticky="nwe")
    dire_button.configure(bd=0, relief="flat")

    cambia_colore = tk.Button(menu, text="Change Color", font=("Helvetica", 11), command=change_frame, bg=sfondo, fg=scritte, activebackground=sfondo, activeforeground=scritte)
    cambia_colore.grid(row=1, column=0, padx=3, pady=3, sticky="sew")
    cambia_colore.configure(bd=0, relief="flat")

    esci = tk.Button(menu, text="Exit", font=("Helvetica", 11), command=uscita, bg=sfondo, fg=scritte, activebackground=sfondo, activeforeground=scritte)
    esci.grid(row=2, column=0, padx=3, pady=3, sticky="sew")
    esci.configure(bd=0, relief="flat")
  
    togli_menu = tk.Button(menu, text="◄", font=("Helvetica", 11), command=elimina_menu, bg=sfondo, fg=scritte, activebackground=sfondo, activeforeground=scritte)
...
#    a = tk.Label(finestra_guida, text="\nFirst of all you have to login into your Instagram account (only if you want to download pictures from a private profile). ", font=("Helvetica", 11), bg=sfondo, fg=scritte).grid(row=0, column=1)
#    b = tk.Label(finestra_guida, text="Next you have to type the name of the profile under <Download Posts>.", font=("Helvetica", 11), bg=sfondo, fg=scritte).grid(row=1, column=1)
#    z = tk.Label(finestra_guida, text="The default download directory is where you have the folder with the "".exe"" file of Instagram Downloader.", font=("Helvetica", 11), bg=sfondo, fg=scritte).grid(row=2, column=1)
#    f = tk.Label(finestra_guida, text="(You can change the directory in the menu)", font=("Helvetica", 11), bg=sfondo, fg=scritte).grid(row=3, column=1)
...
def mostra_login():
    ...
    img_sfondo_3 = ImageTk.PhotoImage(Image.open("insta.png"))
    img_sfondo_4 = img_sfondo_3
    ...
    login_scritta = tk.Label(login_frame, text="LOGIN", font=("Helvetica", 11), bg="#f0f0f0", fg=scritte).grid(row=0, column=1, sticky="nwe")
  
    nome_utente = tk.Label(login_frame, text="Username", font=("Helvetica", 11), bg="#f0f0f0", fg=scritte).grid(row=1, column=0, sticky="w")
  
    password = tk.Label(login_frame, text="Password", font=("Helvetica", 11), bg="#f0f0f0", fg=scritte).grid(row=2, column=0, sticky="w")      
...
def view_bt(event):
    ...
    show_bt = tk.Button(login_frame, text="Show", relief="flat", bd=0, command=hide_bt_fc, bg="#f0f0f0", fg=scritte, activebackground=sfondo, activeforeground=scritte)
...
def view_bt_2():
  global show_bt
  psw_entry.config(show="*")
  show_bt = tk.Button(login_frame, text="Show", relief="flat", command=hide_bt_fc, bd=0, bg="#f0f0f0", fg=scritte, activebackground=sfondo, activeforeground=scritte)
  show_bt.grid(row=2, column=2, sticky="w", pady=10, padx=2)
...
def insta_frame():
    ...
    frame_insta.img = ImageTk.PhotoImage(Image.open("insta_img_4.jpeg"))
    ...
    post = tk.Label(frame_insta, text="Download Posts", font=("Helvetica", 11), bg=sfondo, fg=scritte).grid(row=0, column=0, pady=3, sticky="n")
...
def file_col_1(event):
    colore11 = colore1.get()
    file_colore = open("colore-evidenza.txt", "w").write(colore11)
    messagebox.showinfo("Background Color Saved", "The background color has been saved. Restart the app to apply the changes")

def file_col_2(event):
    colore22 = colore2.get()
    file_colore = open("colore-scritte.txt", "w").write(colore22)
    messagebox.showinfo("Font Color Saved", "The font color has been saved. Restart the app to apply the changes")

def change_frame():
    global colore1, colore2

    frame_change = tk.Toplevel(bg=sfondo)
    frame_change.wm_attributes("-topmost", True)
    frame_change.resizable(0,0)

    colore_sfondo = tk.StringVar()
    colore_scritte = tk.StringVar()

    sfondo_colore = tk.Label(frame_change, text="Background Color", font=("Helvetica", 11), bg=sfondo, fg=scritte).grid(row=0, column=0, sticky="ns")
    colore1 = tk.Entry(frame_change, textvariable=colore_sfondo, bg=sfondo, fg=scritte)
    colore1.grid(row=2, column=0, pady=10, padx=1)
    hex_1 = tk.Label(frame_change, text="(es. #000000)", font=("Helvetica", 11), bg=sfondo, fg=scritte).grid(row=1, column=0, sticky="ns")

    scritte_colore = tk.Label(frame_change, text="Font Color", font=("Helvetica", 11), bg=sfondo, fg=scritte).grid(row=0, column=1, sticky="ns")
    colore2 = tk.Entry(frame_change, textvariable=colore_scritte, bg=sfondo, fg=scritte)
    colore2.grid(row=2, column=1, pady=10, padx=10)
    hex_2 = tk.Label(frame_change, text="(es. #ffffff)", font=("Helvetica", 11), bg=sfondo, fg=scritte).grid(row=1, column=1, sticky="ns")
  
    colore1.bind("<KeyPress-Return>", file_col_1)
    colore2.bind("<KeyPress-Return>", file_col_2)